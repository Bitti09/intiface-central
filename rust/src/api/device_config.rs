...
impl ExposedServerDeviceDefinition {
  ...
  pub fn update_feature(&mut self, feature: &ExposedServerDeviceFeature) {
    if self.definition.features().values().any(|x| x.id() == feature.id()) {
      self.definition = ServerDeviceDefinitionBuilder::from_user(&self.definition).replace_feature(&feature.feature).finish();
    }
  }
  ...
  pub fn update_feature_output_properties(&mut self, props: &ExposedServerDeviceFeatureOutputProperties) {
    if let Some(f) = self.definition.features().values().find(|x| x.id() == props.feature_id) {
      ...
      if !f.output.is_empty() {
        info!("Has output");
        let mut output = f.output.clone();
        if output.contains_key(&props.output_type) {
          ...
          output.retain(|x| x.output_type() != props.output_type);
...
impl ExposedServerDeviceFeature {
  ...
  pub fn set_output(&mut self, output: Option<ExposedServerDeviceFeatureOutput>) {
    if let Some(o) = output {
      self.feature.output = o.output;
    } else {
      self.feature.output = Default::default();
    }
  }
  ...
  pub fn input(&self) -> Option<ExposedServerDeviceFeatureInput> {
    if self.feature.input.is_empty() {
      ...
      Some(ExposedServerDeviceFeatureInput::from(self.feature.input.clone()))
    }
  }
}
...
impl ExposedServerDeviceFeatureOutput {
  fn new(feature_id: Uuid, value: SmallVecEnumMap<ServerDeviceFeatureOutput, 1>) -> Self {
    Self {
      feature_id,
      output: value
    }
  ...
  pub fn vibrate(&self) -> Option<ExposedServerDeviceFeatureOutputProperties> {
    self.output.find_by_key(&OutputType::Vibrate).and_then(|x| x.as_value_properties()).map(|x| ExposedServerDeviceFeatureOutputProperties::new_from_value(self.feature_id, OutputType::Vibrate, x.clone()))
  }

  #[frb(sync, getter)]
  pub fn rotate(&self) -> Option<ExposedServerDeviceFeatureOutputProperties> {
    self.output.find_by_key(&OutputType::Rotate).and_then(|x| x.as_value_properties()).map(|x| ExposedServerDeviceFeatureOutputProperties::new_from_value(self.feature_id, OutputType::Rotate, x.clone()))
  }

  #[frb(sync, getter)]
  pub fn oscillate(&self) -> Option<ExposedServerDeviceFeatureOutputProperties> {
    self.output.find_by_key(&OutputType::Oscillate).and_then(|x| x.as_value_properties()).map(|x| ExposedServerDeviceFeatureOutputProperties::new_from_value(self.feature_id, OutputType::Oscillate, x.clone()))
  }

  #[frb(sync, getter)]
  pub fn constrict(&self) -> Option<ExposedServerDeviceFeatureOutputProperties> {
    self.output.find_by_key(&OutputType::Constrict).and_then(|x| x.as_value_properties()).map(|x| ExposedServerDeviceFeatureOutputProperties::new_from_value(self.feature_id, OutputType::Constrict, x.clone()))
  }

  #[frb(sync, getter)]
  pub fn temperature(&self) -> Option<ExposedServerDeviceFeatureOutputProperties> {
    self.output.find_by_key(&OutputType::Temperature).and_then(|x| x.as_value_properties()).map(|x| ExposedServerDeviceFeatureOutputProperties::new_from_value(self.feature_id, OutputType::Temperature, x.clone()))
  }

  #[frb(sync, getter)]
  pub fn led(&self) -> Option<ExposedServerDeviceFeatureOutputProperties> {
    self.output.find_by_key(&OutputType::Led).and_then(|x| x.as_value_properties()).map(|x| ExposedServerDeviceFeatureOutputProperties::new_from_value(self.feature_id, OutputType::Led, x.clone()))
  }

  #[frb(sync, getter)]
  pub fn spray(&self) -> Option<ExposedServerDeviceFeatureOutputProperties> {
    self.output.find_by_key(&OutputType::Spray).and_then(|x| x.as_value_properties()).map(|x| ExposedServerDeviceFeatureOutputProperties::new_from_value(self.feature_id, OutputType::Spray, x.clone()))
  }

  #[frb(sync, getter)]
  pub fn position(&self) -> Option<ExposedServerDeviceFeatureOutputProperties> {
    if let Some(ServerDeviceFeatureOutput::Position(p)) = self.output.find_by_key(&OutputType::Position) {
  ...
  pub fn position_with_duration(&self) -> Option<ExposedServerDeviceFeatureOutputProperties> {
    if let Some(ServerDeviceFeatureOutput::HwPositionWithDuration(p)) = self.output.find_by_key(&OutputType::HwPositionWithDuration) {
...
impl From<SmallVecEnumMap<ServerDeviceFeatureInput, 1>> for ExposedServerDeviceFeatureInput {
  fn from(value: SmallVecEnumMap<ServerDeviceFeatureInput, 1>) -> Self {
    Self {
      input: value
    }
  }
...
impl From<&RangeWithLimit> for ExposedRangeWithLimit {
  fn from(value: &RangeWithLimit) -> Self {
    Self {
      base: std::ops::RangeInclusive::new(value.base.start().clone(), value.base.end().clone()),
      user: value.user.as_ref().map(|x| std::ops::RangeInclusive::new(x.start().clone(), x.end().clone()))
    }
  }
}
...
impl From<ExposedRangeWithLimit> for RangeWithLimit {
  fn from(value: ExposedRangeWithLimit) -> Self {
    let b = buttplug_core::util::range::RangeInclusive::new(*value.base.start(), *value.base.end());
    let u = value.user.as_ref().map(|x| buttplug_core::util::range::RangeInclusive::new(*x.start(), *x.end()));
    RangeWithLimit::new_with_user(b, u)
  }
}
